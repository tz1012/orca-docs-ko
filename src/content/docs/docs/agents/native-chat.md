---
title: "채팅 UI(네이티브 채팅)"
sourceUrl: https://www.onorca.dev/docs/agents/native-chat
checkedAt: "2026-07-29T01:03:00.276Z"
editUrl: false
prev: /orca-docs-ko/docs/agents/codex-hot-swap/
next: /orca-docs-ko/docs/agents/session-history/
translationNotice:
  title: "비공식 한국어 번역"
  message: "이 문서는 ORCA 공식 문서의 비공식 한국어 번역입니다. 내용이 다를 경우 원문이 우선합니다."
  rights: "원본 문서와 이미지의 권리는 Lovecast Inc. 및 각 권리자에게 있습니다."
---

지원되는 에이전트 터미널 위에 선택적으로 제공되는 채팅 인터페이스로, 스킬, 모델 선택기 및 대화 기록 보기를 지원합니다.

채팅 UI는 지원되는 에이전트 터미널 세션 위에 계층으로 제공되는 실험적 보기입니다. 터미널이 계속 기준 정보이며, 채팅 UI는 동일한 PTY를 위한 구조화된 대화 기록과 작성기입니다.

## 활성화

1.  **`Settings`(설정) → `Experimental`(실험) → `Chat UI`(채팅 UI)**를 엽니다.
2.  **`Chat UI`(채팅 UI)**를 켭니다.
3.  선택적으로 **`Default view`(기본 보기)**를 **`Chat UI`(채팅 UI)**로 설정하여 지원되는 새 에이전트 탭이 채팅으로 열리게 합니다. TUI를 먼저 열려면 **`Terminal`(터미널)**을 유지합니다.

활성화한 후 에이전트 창에서 `Chat UI`(채팅 UI) ↔ 터미널을 전환합니다.

## 작성기

-   메시지를 입력해 전송하거나 진행 중인 턴을 중지합니다. 호스트가 지원하면 files/images을 첨부합니다.
-   슬래시 명령과 검색된 **스킬**을 사용하려면 `/`를 입력합니다(가능한 경우 에이전트별로 필터링됩니다).
-   에이전트가 제공하는 경우 **`model`(모델)** 및 옵션 필(사고 수준, 모드, 에이전트별 세션 옵션)을 사용합니다. 값은 실행 중인 CLI에 전달되며 일부 옵션은 세션이 시작된 후에만 적용됩니다.

## 제공 범위

채팅 UI는 지원되는 로컬 에이전트 세션용 데스크톱 앱에서 제공됩니다. [모바일 컴패니언](/orca-docs-ko/docs/mobile/)은 동일하게 페어링된 세션에 채팅 형식의 대화 기록 패턴을 재사용합니다.

실험적 기능

대화 기록의 충실도, 스트리밍 및 터미널과의 기능 일치는 계속 조정 중입니다. OSC/status의 모든 세부 정보가 필요하면 원시 TUI를 사용하는 것이 좋습니다.
