...
-   **`New Linear issue`(새 Linear 이슈)** 및 **`New Linear project`(새 Linear 프로젝트)** 대화 상자를 실수로 닫더라도(`Escape`(Esc 키), `Cancel`(취소), 바깥쪽 클릭 또는 닫기) title/description을 유지합니다(프로젝트의 경우 이름, 요약 및 개요도 포함). 같은 앱 세션에서 대화 상자를 다시 열면 텍스트가 복원됩니다. 초안은 생성에 성공하면 지워지며 앱을 다시 시작한 뒤에는 유지되지 않습니다. Team/project 선택기는 평소처럼 대화 상자를 열 때의 기본값을 사용합니다.
