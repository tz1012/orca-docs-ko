...
-   작업 트리는 프로젝트 아래에 중첩되고, 에이전트는 작업 트리 링에 배치됩니다. Orca가 parent/child 작업 공간을 알고 있으면 계보도 포함됩니다. 디스패치가 아직 활성 상태이거나 최근에 완료된 경우 오케스트레이션 상위 → 하위 연결선이 같은 작업 트리 또는 서로 다른 작업 트리의 에이전트 카드 사이에 그려집니다.
...
-   **`Header`(헤더)** — 에이전트 아이콘, 세션 **대화 이름**(변경한 이름, 생성된 제목, AI Vault/에이전트 세션 제목 또는 작업 트리 이름으로 대체), 상태 글리프를 표시합니다.
