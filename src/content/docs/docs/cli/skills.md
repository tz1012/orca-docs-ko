...
## `Skills`(스킬) 페이지에서 설치 및 삭제

앱 내 설치에서 작업 공간 대상을 요청하면 작업 공간 이름이나 종류(**`worktree`(작업 트리)** / **`folder`(폴더)**)로 선택기를 검색합니다. 에이전트 선택기는 Orca가 항상 포함하는 디렉터리와 선택할 수 있는 추가 에이전트 디렉터리를 구분하므로, 계속하기 전에 정확한 설치 대상을 검토할 수 있습니다.

검색된 스킬을 제거하려면 작업 메뉴를 열고 **`Delete…`(삭제…)**를 선택합니다. 여러 스킬을 제거하려면 페이지의 **`More actions`(추가 작업)** 메뉴를 열고 **`Delete skills…`(스킬 삭제…)**를 선택한 다음 삭제할 수 있는 결과를 선택합니다. Orca는 확인 전에 번들에 포함되었거나, 플러그인에서 관리되거나, 읽기 전용이거나, 제거 가능한 스킬 폴더 밖에 있는 등의 이유로 복사본을 삭제할 수 없는 이유를 설명합니다. 이는 공유 링크로 설치된 복사본을 관리하는 **`Manage installs`(설치 관리)**와 별개입니다.
