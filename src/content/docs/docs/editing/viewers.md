...
미리 보기의 문서 칩은 주소 표시줄 역할도 합니다. 이를 편집하고 웹 URL을 입력하면 해당 탭이 일반 브라우저 탭으로 전환됩니다. 브라우저 탭에서 작업 공간의 HTML 경로를 입력하면 다시 미리 보기로 전환됩니다. 중간에 다른 페이지 기록이 없으면 **`Back`(뒤로)**을 사용해 두 상태 사이를 이동합니다.
